...
@section('title', 'Kelola Bantuan - Petugas')

@section('content')
<style>
    /* Modern Petugas Bantuan Styles */
    ...
    .ptg-stat-box .stat-desc { font-size: 12px; color: #94a3b8; margin-top: 4px; }
    ...
    .ptg-action-btn:hover { background: #2563eb; color: #fff; }
    ...
    .ptg-empty-icon i { font-size: 40px !important; color: #10b981 !important; }
...
<div class="ptg-container">
    <div class="ptg-header-card">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h1>Kelola Bantuan</h1>
                <p>Kelola data bantuan petani - {{ Auth::user()->alamat_desa }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="ptg-stat-box">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value green">{{ $bantuans->count() }}</div>
        ...
        <div class="ptg-stat-box">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value green">{{ $bantuans->where('status', 'Dikirim')->count() }}</div>
                    <div class="stat-desc">✅ Sudah dikirim</div>
                </div>
        ...
        <div class="ptg-stat-box">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value orange">{{ $bantuans->where('status', 'Diproses')->count() }}</div>
    ...
    <div class="ptg-table-card">
        <div class="ptg-table-header">
            <h5><i class="fas fa-table"></i>Data Bantuan Petani</h5>
            <div class="d-flex align-items-center gap-3">
                <a href="{{ route('petugas.export.bantuan.pdf') }}" class="btn btn-light btn-sm" style="border-radius: 8px; font-weight: 600;">
                    <i class="fas fa-file-pdf text-danger me-1"></i> Export PDF
                </a>
                <div class="ptg-search-box">
                    <i class="fas fa-search"></i>
                    <input type="text" id="searchInput" placeholder="Cari bantuan..." onkeyup="searchTable()">
                </div>
            </div>
        </div>
        
        <div class="table-responsive">
            <table class="ptg-table" id="bantuanTable">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Petani</th>
                ...
                <tbody>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="ptg-user-info">
                                    <div class="name">{{ $bantuan->user->name }}</div>
                                    <div class="desa">{{ $bantuan->user->alamat_desa }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="fw-semibold">{{ $bantuan->jenis_bantuan }}</div>
                            @if($bantuan->catatan)
                            <div class="text-muted small">{{ Str::limit($bantuan->catatan, 30) }}</div>
                            @endif
                        </td>
                        <td><span class="ptg-badge info">{{ $bantuan->jumlah }}</span></td>
                        <td>
                            <div class="fw-medium">{{ \Carbon\Carbon::parse($bantuan->tanggal)->format('d/m/Y') }}</div>
                        </td>
                        <td>
                            @if($bantuan->status == 'Dikirim')
                                <span class="ptg-badge success"><i class="fas fa-check"></i> Dikirim</span>
                            @elseif($bantuan->status == 'Diproses')
                                <span class="ptg-badge warning"><i class="fas fa-clock"></i> Diproses</span>
                            @elseif($bantuan->status == 'Ditolak')
                                <span class="ptg-badge danger"><i class="fas fa-times"></i> Ditolak</span>
                            @else
                                <span class="ptg-badge info">{{ $bantuan->status }}</span>
                            @endif
                        </td>
                        <td>
                            <a href="{{ route('petugas.bantuan.show', $bantuan->id) }}" class="ptg-action-btn">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="ptg-empty">
                                <div class="ptg-empty-icon"><i class="fas fa-hand-holding-heart"></i></div>
                                <h5>Belum Ada Data Bantuan</h5>
                                <p>Data bantuan untuk petani di wilayah Anda akan muncul di sini</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
